...
/**
 * Same behavioural suite as {@code InMemorySagaStoreTest}, against the JDBC
 * store: store/load by id and correlation, the owned-values diff on re-store
 * (drop stale, move changed, restore returning), and type-scoped correlation.
 */
class JdbcSagaStoreTest extends AbstractJdbcTest {

    private JdbcSagaStore store;

    @BeforeEach
    void setUp() {
        store = new JdbcSagaStore(db);
    }
    ...
    void storeSaga_setsCanonicalCorrelationIdToFirstValue() {
        store.storeSaga(new OrderSaga("saga-1", "ord-1", "cust-1"));
        assertEquals("ord-1", store.loadSaga("saga-1").orElseThrow().getCorrelationId());
    }
    ...
    void storeSagaWithNullSagaIdValueThrows() {
        assertThrows(IllegalStateException.class,
                () -> store.storeSaga(new OrderSaga(null, "ord-1", "cust-1")));
    }
    ...
    void correlationIndexIsTypeScoped_sameValueDifferentTypes() {
        ...
        assertEquals("o-1", store.loadSagaByCorrelationId("shared-order", "OrderSaga").orElseThrow().getId());
        assertEquals("s-1", store.loadSagaByCorrelationId("shared-order", "ShipmentSaga").orElseThrow().getId());
    }
    ...
    void multipleSagasOfSameType_areIndependentByPrimaryId() {
        ...
        assertEquals("saga-a", store.loadSagaByCorrelationId("ord-a", "OrderSaga").orElseThrow().getId());
        assertEquals("saga-b", store.loadSagaByCorrelationId("ord-b", "OrderSaga").orElseThrow().getId());
    }

    @Test
    void deleteSaga_removesInstanceAndCorrelations() {
        OrderSaga s = new OrderSaga("saga-1", "ord-1", "cust-1");
        store.storeSaga(s);
        store.deleteSaga(s);
        assertTrue(store.loadSaga("saga-1").isEmpty());
        assertTrue(store.loadSagaByCorrelationId("ord-1", "OrderSaga").isEmpty());
        // idempotent
        assertDoesNotThrow(() -> store.deleteSaga(s));
    }
