...
/**
 * Durable read store for the {@code pfm_operation} table. Identical to the kf sample: writes are
 * insert-ignore on the {@code op_id} primary key so a redelivered {@code OperationRecorded} event
 * can never double-count; totals are SQL aggregates computed at query time, so they cannot drift.
 * This idempotency is what lets the cluster IT survive Axon's at-least-once token replay.
 */
@Repository
